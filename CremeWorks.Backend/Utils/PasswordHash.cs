...
public class PasswordHash
{
    public static async Task<string> HashPasswordAsync(string password, byte[] salt)
    {
        ...
        saltedPassword.Position = 0;

        // Hash the concatenated password and salt
        // & Concatenate the salt and hashed password for storage
        byte[] hashedPasswordWithSalt = new byte[SHA256.HashSizeInBytes + salt.Length];
        await SHA256.HashDataAsync(saltedPassword, hashedPasswordWithSalt.AsMemory(0, SHA256.HashSizeInBytes));
        salt.CopyTo(hashedPasswordWithSalt.AsMemory(SHA256.HashSizeInBytes));
